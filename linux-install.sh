...
PATH_KT="/root/AGproxy"

PATH_EXEC="AGproxy"

PATH_CACHE="/root/AGproxy/.cache"

PATH_CONFIG="/root/AGproxy/.env"

PATH_NOHUP="/root/AGproxy/nohup.out"
PATH_ERR="/root/AGproxy/err.log"


PATH_TURN_ON="/etc/profile.d"
PATH_TURN_ON_SH="/etc/profile.d/AG.sh"
...
turn_on() {
    ...
    if [ ! -f "$PATH_TURN_ON_SH" ];then
        ...
        echo '      echo "AGProxy已启动, 无需重复启动"' >> $PATH_TURN_ON_SH
        echo '  elif [ $(id -u) -ne 0 ]; then' >> $PATH_TURN_ON_SH
        echo '      echo "使用ROOT用户登录才能启动AGPROXY"' >> $PATH_TURN_ON_SH
...
installapp() {
    ...
    colorEcho ${GREEN} "开始安装AGproxy_vcu-firs-${VERSION}"
    ...
    wget -P $PATH_KT "${DOWNLOAD_HOST}/AGproxy_vcu-firs-${VERSION}_linux" -O "${PATH_KT}/${PATH_EXEC}" 1>/dev/null

    filterResult $? "拉取程序 AGproxy_vcu-firs-${VERSION}_linux"
...
check_hub() {
    ...
    tail -f /root/AGproxy/nohup.out
}

check_err() {
    colorEcho ${YELLOW} "按住CTRL+C后台运行"
    tail -f /root/AGproxy/err.log
}
...
echo "-------------------------------------------------------"
colorEcho ${GREEN} "欢迎使用AGMinerProxy安装工具, 请输入操作号继续。"
